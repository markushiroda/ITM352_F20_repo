...
const user_data_filename = 'user_data_json';

//check if file exists before reading//
if (fs.existsSync(filename)) {
    stats = fs.statSync(user_data_filename);
    console.log(`user_data_json has ${stats['size']} characters`);
...
app.get("/use_session", function (request, response) {
    console.log('session id is ' + request.session);
...
app.get("/set_cookie", function (request, response) {
    response.cookie('myname', 'Mark', {maxAge: 5*1000});
...
<input type="submit" value="Submit" id="submit">
    ...
    //of all data is valide, write to the user_data_filename and send to invoice
